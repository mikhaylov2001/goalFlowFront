...
async function loadData() {
  try {
    const [goals, habits, wishes] = await Promise.all([
      apiFetch("/api/goals"),
      apiFetch("/api/habits"),
      apiFetch("/api/wishes"),
    ]);
...
function Confetti({ active }) {
  ...
  return (
    <canvas ref={ref} style={{ position: "fixed", inset: 0, zIndex: 9999, pointerEvents: "none", width: "100%", height: "100%" }} />
  );
}
    ...
    <div style={{ position: "relative", width: size, height: size, flexShrink: 0 }}>
      <svg width={size} height={size} style={{ transform: "rotate(-90deg)" }}>
        ...
        <circle cx={size / 2} cy={size / 2} r={r} fill="none" stroke={color} strokeWidth={stroke}
          strokeDasharray={c} strokeDashoffset={c * (1 - clamp(pct, 0, 100) / 100)}
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 999, display: "flex", flexDirection: "column", justifyContent: "flex-end" }} onClick={onClose}>
      ...
      <div onClick={(e) => e.stopPropagation()} style={{
        ...
        {title && <h2 style={{ margin: "8px 0 18px", fontSize: 19, fontWeight: 800 }}>{title}</h2>}
        {children}
      </div>
    </div>
  );
...
function GoalForm({ goal, onSave, onClose }) {
  const [f, sf] = useState(
    goal || {
      id: uid(), title: "", desc: "", cat: "work", prio: "medium",
      deadline: "", reward: "", tasks: [],
      notif: { enabled: false, freq: "daily", time: "09:00", day: "mon" },
    ...
    <>
      ...
      <div style={{ background: "#F8FAFC", borderRadius: 16, padding: 14, marginBottom: 14 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: f.notif.enabled ? 12 : 0 }}>
          ...
          <span style={{ flex: 1, fontSize: 14, fontWeight: 650 }}>Уведомления</span>
          <div onClick={() => un("enabled", !f.notif.enabled)} style={{
            width: 46, height: 28, borderRadius: 14,
            background: f.notif.enabled ? "#6366F1" : "#E2E8F0",
            cursor: "pointer", position: "relative",
          }}>
            <div style={{
              width: 22, height: 22, borderRadius: 11, background: "#fff",
              position: "absolute", top: 3, left: f.notif.enabled ? 21 : 3,
          ...
          <>
            <div style={{ display: "flex", gap: 6, marginBottom: 10 }}>
              {[{ id: "daily", l: "Каждый день" }, { id: "weekly", l: "Раз в неделю" }].map((fr) => (
                <button key={fr.id} onClick={() => un("freq", fr.id)} style={{
                  flex: 1, padding: "9px 0", borderRadius: 12,
                  border: f.notif.freq === fr.id ? "2px solid #6366F1" : "2px solid #E2E8F0",
                  background: f.notif.freq === fr.id ? "#EEF2FF" : "#fff",
                  fontSize: 12, fontWeight: 700, cursor: "pointer",
                  color: f.notif.freq === fr.id ? "#6366F1" : "#94A3B8",
                }}>{fr.l}</button>
              ))}
            </div>
            <div style={{ display: "flex", gap: 10 }}>
              <div style={{ flex: 1 }}>
                ...
                <input type="time" value={f.notif.time} onChange={(e) => un("time", e.target.value)}
                  style={{ ...S.field, padding: "8px 10px", fontSize: 13 }} />
              </div>
              {f.notif.freq === "weekly" && (
                <div style={{ flex: 1 }}>
                  <label style={{ ...S.lbl, fontSize: 10 }}>День</label>
                  <select value={f.notif.day} onChange={(e) => un("day", e.target.value)}
                    style={{ ...S.field, padding: "8px 10px", fontSize: 13 }}>
                    {[["mon", "Пн"], ["tue", "Вт"], ["wed", "Ср"], ["thu", "Чт"], ["fri", "Пт"], ["sat", "Сб"], ["sun", "Вс"]].map(([v, l]) => (
      ...
      <button onClick={() => { if (f.title.trim()) { onSave(f); onClose(); } }} style={{
        ...
        fontSize: 16, fontWeight: 800, cursor: f.title.trim() ? "pointer" : "default", marginTop: 8,
      }}>
...
function GoalCard({ g, onTap, onToggle }) {
  ...
  return (
    <div onClick={onTap} style={{
      background: "#fff", borderRadius: 20, padding: "16px 18px", marginBottom: 12,
      boxShadow: "0 1px 4px rgba(0,0,0,0.04)", opacity: g.done ? 0.55 : 1,
      position: "relative", overflow: "hidden", cursor: "pointer",
    }}>
      <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: 3, background: `linear-gradient(90deg, ${cat.accent}, ${cat.accent}66)` }} />
      <div style={{ display: "flex", gap: 14, alignItems: "center" }}>
        <Ring pct={pct} size={50} stroke={4} color={cat.accent}>
          <span style={{ fontSize: 18 }}>{g.done ? "🎉" : cat.emoji}</span>
        </Ring>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 2 }}>
            <span style={{ fontSize: 10, fontWeight: 700, color: pri.c, background: pri.bg, padding: "2px 7px", borderRadius: 6 }}>{pri.l}</span>
            {g.reward && <span style={{ fontSize: 10, color: "#A855F7" }}>🎁</span>}
            {g.notif?.enabled && <span style={{ fontSize: 10 }}>🔔</span>}
          </div>
          <h3 style={{
            margin: "2px 0 0", fontSize: 16, fontWeight: 750,
          ...
          <div style={{ display: "flex", gap: 8, marginTop: 6, alignItems: "center" }}>
            ...
            {total > 0 && <span style={{ fontSize: 11, fontWeight: 600, color: "#94A3B8" }}>✅ {done}/{total}</span>}
          </div>
        </div>
        <div onClick={(e) => { e.stopPropagation(); onToggle(g.id); }} style={{
          width: 34, height: 34, borderRadius: 11, flexShrink: 0,
          border: g.done ? "none" : "2.5px solid #E2E8F0",
          background: g.done ? `linear-gradient(135deg, ${cat.accent}, ${cat.accent}BB)` : "#fff",
...
function GoalDetail({ goal, onBack, onUpdate, onDelete, onConfetti, onEdit }) {
  ...
  useEffect(() => {
    if (total > 0 && done === total && !goal.done) {
      const t = setTimeout(() => { onUpdate({ ...goal, done: true }); onConfetti(); }, 400);
  ...
  const addTask = async () => {
    ...
    const tid = uid();
    try {
      await apiFetch("/api/tasks", { method: "POST", body: JSON.stringify({ id: tid, goal_id: goal.id, title: newTask.trim(), prio: newTP }) });
  ...
  const toggleTask = async (tid) => {
    const tog = (ts) => ts.map((t) =>
      t.id === tid ? { ...t, done: !t.done, children: t.children.map((c) => ({ ...c, done: !t.done })) } : { ...t, children: tog(t.children) }
    );
    const updated = tog(goal.tasks);
    const task = goal.tasks.find((t) => t.id === tid);
    if (task) {
      try {
        await apiFetch(`/api/tasks/${tid}`, { method: "PUT", body: JSON.stringify({ done: !task.done }) });
        for (const c of task.children) {
          await apiFetch(`/api/microtasks/${c.id}`, { method: "PUT", body: JSON.stringify({ done: !task.done }) });
        }
      } catch (e) { console.error(e); }
    }
  ...
  const toggleMicro = async (tid, mid) => {
    const upd = (ts) => ts.map((t) => {
      if (t.id === tid) {
        const nc = t.children.map((c) => c.id === mid ? { ...c, done: !c.done } : c);
    ...
    if (micro) {
      try {
        await apiFetch(`/api/microtasks/${mid}`, { method: "PUT", body: JSON.stringify({ done: !micro.done }) });
        const nc = task.children.map((c) => c.id === mid ? { ...c, done: !c.done } : c);
        const allDone = nc.length > 0 && nc.every((c) => c.done);
        if (allDone !== task.done) {
          await apiFetch(`/api/tasks/${tid}`, { method: "PUT", body: JSON.stringify({ done: allDone }) });
        }
      } catch (e) { console.error(e); }
    }
    onUpdate({ ...goal, tasks: updated });
  ...
  const deleteTask = async (tid) => {
    try { await apiFetch(`/api/tasks/${tid}`, { method: "DELETE" }); } catch (e) { console.error(e); }
  ...
  const addMicro = async (pid, title) => {
    ...
    const mid = uid();
    try {
      await apiFetch("/api/microtasks", { method: "POST", body: JSON.stringify({ id: mid, task_id: pid, title: title.trim() }) });
    } catch (e) { console.error(e); }
    onUpdate({
      ...goal,
      tasks: goal.tasks.map((t) =>
        t.id === pid ? { ...t, children: [...t.children, { id: mid, title: title.trim(), done: false }], done: false } : t
      ),
    });
  };

  const setTPrio = async (tid, p) => {
    try { await apiFetch(`/api/tasks/${tid}`, { method: "PUT", body: JSON.stringify({ prio: p }) }); } catch (e) { console.error(e); }
  ...
  const sorted = useMemo(() => {
    ...
    if (sortBy === "priority") {
      arr.sort((a, b) => {
        const pa = { high: 0, medium: 1, low: 2 }[a.prio || "medium"];
        const pb = { high: 0, medium: 1, low: 2 }[b.prio || "medium"];
    ...
    <div style={{ animation: "pageIn .3s ease" }}>
      ...
      <div style={{ background: `linear-gradient(135deg, ${cat.accent}, ${cat.accent}CC)`, padding: "calc(env(safe-area-inset-top, 16px) + 8px) 20px 24px", color: "#fff", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 16, position: "relative" }}>
          ...
          <div style={{ flex: 1 }} />
          <button onClick={() => onEdit(goal)} style={{ background: "rgba(255,255,255,0.15)", border: "none", color: "#fff", fontSize: 13, cursor: "pointer", padding: "8px 14px", borderRadius: 12, fontWeight: 700 }}>✏️</button>
          <button onClick={() => onDelete(goal.id)} style={{ background: "rgba(255,255,255,0.15)", border: "none", color: "#fff", fontSize: 13, cursor: "pointer", padding: "8px 14px", borderRadius: 12, fontWeight: 700 }}>🗑️</button>
        </div>

        <div style={{ position: "relative" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 6 }}>
            <span style={{ fontSize: 28 }}>{cat.emoji}</span>
            <span style={{ fontSize: 11, fontWeight: 700, background: "rgba(255,255,255,0.2)", padding: "3px 10px", borderRadius: 8 }}>{cat.label}</span>
            <span style={{ fontSize: 11, fontWeight: 700, background: "rgba(255,255,255,0.2)", padding: "3px 10px", borderRadius: 8 }}>{pri.l}</span>
          </div>
          <h1 style={{ margin: "4px 0 0", fontSize: 24, fontWeight: 900, letterSpacing: "-0.03em", lineHeight: 1.2 }}>{goal.title}</h1>
          {goal.desc && <p style={{ margin: "8px 0 0", fontSize: 13, opacity: 0.8, lineHeight: 1.5 }}>{goal.desc}</p>}
        </div>

        {/* Stats */}
        <div style={{ display: "flex", gap: 10, marginTop: 20, position: "relative" }}>
          <div style={{ flex: 1, background: "rgba(255,255,255,0.15)", borderRadius: 16, padding: "12px 14px" }}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
              <span style={{ fontSize: 12, fontWeight: 700, opacity: 0.8 }}>Прогресс</span>
              <span style={{ fontSize: 18, fontWeight: 900 }}>{pct}%</span>
            </div>
            <div style={{ height: 6, borderRadius: 3, background: "rgba(255,255,255,0.2)", overflow: "hidden" }}>
              <div style={{ height: "100%", borderRadius: 3, width: `${pct}%`, background: "#fff", transition: "width .5s" }} />
            </div>
            <div style={{ fontSize: 11, marginTop: 6, opacity: 0.7, fontWeight: 600 }}>{done} из {total}</div>
          </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {goal.deadline && (
                <div style={{ background: "rgba(255,255,255,0.15)", borderRadius: 14, padding: "10px 14px", textAlign: "center" }}>
                  ...
                  {dl !== null && <div style={{ fontSize: 10, opacity: 0.7 }}>{dl < 0 ? "просрочено!" : dl === 0 ? "сегодня!" : `${dl} дн.`}</div>}
                </div>
              )}
              {goal.reward && (
                <div style={{ background: "rgba(255,255,255,0.15)", borderRadius: 14, padding: "10px 14px", textAlign: "center" }}>
      ...
      <div style={{ padding: "20px 16px calc(env(safe-area-inset-bottom, 20px) + 20px)" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
          ...
          <button onClick={() => setSortBy((s) => s === "priority" ? "default" : "priority")} style={{
            background: sortBy === "priority" ? `${cat.accent}14` : "#F8FAFC",
            border: "none", padding: "6px 12px", borderRadius: 10,
            fontSize: 11, fontWeight: 700, cursor: "pointer",
            color: sortBy === "priority" ? cat.accent : "#94A3B8",
          }}>{sortBy === "priority" ? "🔽 По приоритету" : "📋 По порядку"}</button>
        </div>
        ...
        <div style={{ display: "flex", gap: 10, marginBottom: 6 }}>
          <input value={newTask} onChange={(e) => setNewTask(e.target.value)}
            ...
            placeholder="＋ Новая задача..." style={{
              flex: 1, padding: "14px 16px", borderRadius: 16, border: "2px solid #F1F5F9",
              fontSize: 15, outline: "none", background: "#fff", color: "#0F172A", boxSizing: "border-box",
            }} />
        ...
        <div style={{ display: "flex", gap: 6, marginBottom: 16 }}>
          {TPRIO.map((p) => (
            <button key={p.id} onClick={() => setNewTP(p.id)} style={{
              padding: "5px 10px", borderRadius: 50,
              border: newTP === p.id ? `2px solid ${p.c}` : "2px solid #F1F5F9",
              background: newTP === p.id ? p.c + "14" : "#FAFAFA",
              fontSize: 10, fontWeight: 700, cursor: "pointer", color: newTP === p.id ? p.c : "#B0B8C4",
            }}>{p.e} {p.l}</button>
          ))}
        </div>
        ...
        {goal.tasks.length === 0 ? (
          <div style={{ textAlign: "center", padding: "40px 20px" }}>
            ...
            <div style={{ fontSize: 15, fontWeight: 700, color: "#94A3B8" }}>Добавь задачу</div>
          </div>
        ) : (
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {sorted.map((task) => {
              ...
              return (
                <TaskItem key={task.id} task={task} tp={tp} accent={cat.accent}
                  isExp={isExp}
                  onToggleExp={() => setExp((p) => ({ ...p, [task.id]: !p[task.id] }))}
...
/* ── HabitsPage ── */
function HabitsPage({ habits, onUpdate, onReload }) {
  ...
  const toggleDay = async (id, dk) => {
    try {
      ...
      onUpdate(habits.map((h) => {
        if (h.id !== id) return h;
        const logs = { ...h.logs };
        if (logs[dk]) delete logs[dk]; else logs[dk] = true;
  ...
  const saveH = async (h) => {
    try {
      ...
      if (ex) {
        await apiFetch(`/api/habits/${h.id}`, { method: "PUT", body: JSON.stringify({ title: h.title, emoji: h.emoji, color: h.color, streak: ex.streak || 0 }) });
        onUpdate(habits.map((x) => x.id === h.id ? { ...x, title: h.title, emoji: h.emoji, color: h.color } : x));
      } else {
        await apiFetch("/api/habits", { method: "POST", body: JSON.stringify(h) });
  ...
  const delH = async (id) => {
    try { await apiFetch(`/api/habits/${id}`, { method: "DELETE" }); } catch (e) { console.error(e); }
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
        <h2 style={{ margin: 0, fontSize: 20, fontWeight: 900 }}>Привычки</h2>
        <button onClick={() => { setEditH(null); setSheet(true); }} style={{
          width: 40, height: 40, borderRadius: 12, border: "none",
          background: "linear-gradient(135deg, #10B981, #059669)", color: "#fff", fontSize: 20, cursor: "pointer",
          display: "flex", alignItems: "center", justifyContent: "center",
        }}>+</button>
      </div>
      {habits.length === 0 ? (
        <div style={{ textAlign: "center", padding: "50px 20px" }}>
          <div style={{ fontSize: 48 }}>🔄</div>
          <div style={{ fontSize: 15, fontWeight: 700, color: "#94A3B8", marginTop: 8 }}>Добавь привычку</div>
        </div>
      ) : (
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          {habits.map((h) => (
            <div key={h.id} style={{
              ...
              border: `1px solid ${h.color}18`, position: "relative", overflow: "hidden",
            }}>
              <div style={{ height: 3, background: `linear-gradient(90deg, ${h.color}, ${h.color}55)`, position: "absolute", top: 0, left: 0, right: 0 }} />
              <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 14 }}>
                {h.emoji && (
                  <div style={{ width: 44, height: 44, borderRadius: 14, background: h.color + "14", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 22 }}>{h.emoji}</div>
                )}
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 16, fontWeight: 750 }}>{h.title}</div>
                  <div style={{ fontSize: 12, fontWeight: 700, color: h.color, marginTop: 2 }}>
                    🔥 {h.streak || 0} <span style={{ fontWeight: 500, color: "#94A3B8" }}>дней подряд</span>
                  </div>
                </div>
                <button onClick={() => { setEditH(h); setSheet(true); }} style={{ background: "none", border: "none", fontSize: 12, cursor: "pointer", color: "#CBD5E1" }}>✏️</button>
                <button onClick={() => delH(h.id)} style={{ background: "none", border: "none", fontSize: 14, cursor: "pointer", color: "#E2E8F0" }}>✕</button>
              </div>
              <div style={{ display: "flex", gap: 6, justifyContent: "center" }}>
                {week.map((dk, i) => {
                  const active = !!h.logs[dk];
                  const isToday = dk === tk;
                  return (
                    <div key={dk} onClick={() => toggleDay(h.id, dk)} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 4, cursor: "pointer" }}>
                      <div style={{
                        width: 34, height: 34, borderRadius: 10,
                        background: active ? h.color : "#F1F5F9",
                        display: "flex", alignItems: "center", justifyContent: "center",
                        fontSize: 12, color: active ? "#fff" : "#CBD5E1", fontWeight: 800,
                        border: isToday && !active ? `2px solid ${h.color}55` : "2px solid transparent",
                      }}>{active ? "✓" : ""}</div>
                      <span style={{ fontSize: 10, color: isToday ? "#0F172A" : "#94A3B8", fontWeight: isToday ? 800 : 600 }}>{WEEKDAYS[i]}</span>
                    </div>
                  );
                })}
              </div>
      ...
      <Sheet open={sheet} onClose={() => { setSheet(false); setEditH(null); }} title={editH ? "Редактировать" : "Новая привычка"}>
...
function WishesPage({ wishes, onUpdate }) {
  ...
  const saveW = async (w) => {
    try {
      ...
      if (ex) {
        await apiFetch(`/api/wishes/${w.id}`, { method: "PUT", body: JSON.stringify(w) });
        onUpdate(wishes.map((x) => x.id === w.id ? w : x));
      } else {
        await apiFetch("/api/wishes", { method: "POST", body: JSON.stringify(w) });
        onUpdate([...wishes, w]);
      }
    } catch (e) { console.error(e); }
  ...
  const toggleW = async (id) => {
    ...
    if (!w) return;
    try {
      await apiFetch(`/api/wishes/${id}`, { method: "PUT", body: JSON.stringify({ ...w, done: !w.done }) });
    } catch (e) { console.error(e); }
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
        <h2 style={{ margin: 0, fontSize: 20, fontWeight: 900 }}>Желания</h2>
        <button onClick={() => { setEditW(null); setSheet(true); }} style={{
          width: 40, height: 40, borderRadius: 12, border: "none",
          background: "linear-gradient(135deg, #EC4899, #DB2777)", color: "#fff", fontSize: 20, cursor: "pointer",
          display: "flex", alignItems: "center", justifyContent: "center",
        }}>+</button>
      </div>
      {wishes.length === 0 ? (
        <div style={{ textAlign: "center", padding: "50px 20px" }}>
          <div style={{ fontSize: 48 }}>💫</div>
          <div style={{ fontSize: 15, fontWeight: 700, color: "#94A3B8", marginTop: 8 }}>Добавь мечту</div>
        </div>
      ) : (
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {wishes.map((w) => (
            <div key={w.id} style={{
              background: w.done ? "#F0FDF4" : "#fff", borderRadius: 18, padding: "14px 18px",
              boxShadow: "0 1px 4px rgba(0,0,0,0.04)",
              border: w.done ? "1px solid #86EFAC" : "1px solid #F1F5F9",
              display: "flex", alignItems: "center", gap: 14,
            }}>
              <div onClick={() => toggleW(w.id)} style={{
                width: 30, height: 30, borderRadius: 10, flexShrink: 0,
                border: w.done ? "none" : "2.5px solid #E2E8F0",
                background: w.done ? "linear-gradient(135deg, #10B981, #059669)" : "#fff",
                cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center",
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{
                  ...
                  color: w.done ? "#059669" : "#1E293B",
                  textDecoration: w.done ? "line-through" : "none",
                }}>{w.emoji && <span style={{ marginRight: 6 }}>{w.emoji}</span>}{w.title}</div>
                {w.note && <div style={{ fontSize: 12, color: "#94A3B8", marginTop: 2, lineHeight: 1.3 }}>{w.note}</div>}
              </div>
              <button onClick={() => { setEditW(w); setSheet(true); }} style={{ background: "none", border: "none", fontSize: 12, cursor: "pointer", color: "#CBD5E1", padding: 4 }}>✏️</button>
              <button onClick={() => delW(w.id)} style={{ background: "none", border: "none", color: "#E2E8F0", fontSize: 14, cursor: "pointer", padding: 4 }}>✕</button>
            </div>
          ))}
        </div>
      )}
...
function CalView({ goals }) {
  ...
  const gmap = useMemo(() => {
    const mp = {};
    goals.forEach((g) => { if (g.deadline) (mp[g.deadline] = mp[g.deadline] || []).push(g); });
    return mp;
  }, [goals]);

  const upcoming = useMemo(() =>
    goals.filter((g) => !g.done && g.deadline)
      .sort((a, b) => {
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
        <button onClick={() => setCur(new Date(y, m - 1, 1))} style={S.ghostBtn}>◀</button>
        <span style={{ fontWeight: 800, fontSize: 17 }}>{MO_NAMES[m]} {y}</span>
        <button onClick={() => setCur(new Date(y, m + 1, 1))} style={S.ghostBtn}>▶</button>
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(7,1fr)", gap: 3, marginBottom: 24 }}>
        {WEEKDAYS.map((d) => <div key={d} style={{ textAlign: "center", fontSize: 11, fontWeight: 700, color: "#94A3B8", padding: 4 }}>{d}</div>)}
        {days.map(({ d, cur: cm }, i) => {
          const k = dateKey(d);
          const dg = gmap[k] || [];
          ...
          return (
            <div key={i} style={{
              minHeight: 50, padding: 4, borderRadius: 12,
              background: td ? "#EEF2FF" : cm ? "#fff" : "#FAFAFA",
              border: td ? "2px solid #6366F1" : "1px solid #F1F5F9",
              opacity: cm ? 1 : 0.25,
            }}>
              <div style={{ fontSize: 11, fontWeight: td ? 800 : 500, color: td ? "#6366F1" : "#64748B", marginBottom: 2 }}>{d.getDate()}</div>
              {dg.slice(0, 2).map((g) => {
                const ct = CATS.find((c) => c.id === g.cat);
                return <div key={g.id} style={{
                  fontSize: 9, padding: "1px 4px", borderRadius: 4, marginBottom: 1,
                  background: ct?.accent + "18", color: ct?.accent, fontWeight: 700,
                  whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis",
                  textDecoration: g.done ? "line-through" : "none",
                }}>{g.title}</div>;
              })}
      ...
      <h3 style={{ fontSize: 16, fontWeight: 800, marginBottom: 12 }}>📋 Дедлайны <span style={{ fontSize: 12, fontWeight: 600, color: "#94A3B8" }}>(по важности)</span></h3>
      {upcoming.length === 0 ? (
        <div style={{ textAlign: "center", padding: "20px", color: "#94A3B8", fontSize: 13 }}>Нет дедлайнов</div>
      ) : (
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {upcoming.map((g) => {
            const cat = CATS.find((c) => c.id === g.cat) || CATS[0];
            const dl = daysLeft(g.deadline);
            const overdue = dl < 0;
            const pri = PRIO[g.prio];
            return (
              <div key={g.id} style={{
                display: "flex", alignItems: "center", gap: 12, padding: "12px 14px",
                borderRadius: 14, background: "#fff", boxShadow: "0 1px 3px rgba(0,0,0,0.04)",
                border: overdue ? "1px solid #FCA5A5" : "1px solid #F1F5F9",
                borderLeft: `4px solid ${pri.c}`,
              }}>
                <span style={{ fontSize: 22 }}>{cat.emoji}</span>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 14, fontWeight: 700, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{g.title}</div>
                  <div style={{ fontSize: 11, color: "#94A3B8", marginTop: 2 }}>{fmtDateFull(g.deadline)}</div>
                </div>
                <span style={{
                  fontSize: 12, fontWeight: 800, padding: "4px 10px", borderRadius: 8, whiteSpace: "nowrap",
                  background: overdue ? "#FEE2E2" : dl <= 3 ? "#FEF3C7" : "#F1F5F9",
                  color: overdue ? "#DC2626" : dl <= 3 ? "#D97706" : "#64748B",
                }}>
...
/* ── NotifPage ── */
function NotifPage({ goals }) {
  const withN = goals.filter((g) => g.notif?.enabled && !g.done);
  const dayN = { mon: "Понедельник", tue: "Вторник", wed: "Среда", thu: "Четверг", fri: "Пятница", sat: "Суббота", sun: "Воскресенье" };
  ...
  return (
    <div>
      <h2 style={{ fontSize: 20, fontWeight: 900, marginBottom: 16 }}>Уведомления</h2>
      {soon.length > 0 && (
        <div style={{ background: "linear-gradient(135deg, #FEF3C7, #FDE68A33)", borderRadius: 20, padding: 18, marginBottom: 16 }}>
          <div style={{ fontSize: 14, fontWeight: 800, color: "#92400E", marginBottom: 12 }}>⏰ Скоро дедлайн</div>
          {soon.map((g) => {
            const d = daysLeft(g.deadline);
            return (
              <div key={g.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 0", borderBottom: "1px solid #FDE68A44" }}>
                <span style={{ fontSize: 14, fontWeight: 600 }}>{CATS.find((c) => c.id === g.cat)?.emoji} {g.title}</span>
                <span style={{ fontSize: 12, fontWeight: 800, padding: "3px 10px", borderRadius: 8, background: d <= 1 ? "#FEE2E2" : "#FEF3C7", color: d <= 1 ? "#DC2626" : "#D97706" }}>
                  {d === 0 ? "Сегодня!" : d === 1 ? "Завтра" : `${d} дн.`}
                </span>
              </div>
            );
          })}
      ...
      <h3 style={{ fontSize: 15, fontWeight: 800, marginBottom: 12 }}>📅 Расписание</h3>
      {withN.length === 0 ? (
        <div style={{ textAlign: "center", padding: "30px 20px", color: "#94A3B8" }}>
          <div style={{ fontSize: 36, marginBottom: 8 }}>😴</div>
          <div style={{ fontSize: 13, fontWeight: 600 }}>Нет уведомлений</div>
        </div>
      ) : (
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {withN.map((g) => {
            const cat = CATS.find((c) => c.id === g.cat) || CATS[0];
            return (
              <div key={g.id} style={{ background: "#fff", borderRadius: 16, padding: "14px 16px", boxShadow: "0 1px 3px rgba(0,0,0,0.04)", border: `1px solid ${cat.accent}18` }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
                  <span style={{ fontSize: 18 }}>{cat.emoji}</span>
                  <span style={{ fontSize: 14, fontWeight: 700, flex: 1 }}>{g.title}</span>
                </div>
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                  <span style={{ fontSize: 12, fontWeight: 700, padding: "4px 10px", borderRadius: 8, background: "#EEF2FF", color: "#6366F1" }}>🕐 {g.notif.time}</span>
                  <span style={{ fontSize: 12, fontWeight: 700, padding: "4px 10px", borderRadius: 8, background: "#F0FDF4", color: "#10B981" }}>
                    {g.notif.freq === "daily" ? "Каждый день" : `Каждый ${dayN[g.notif.day] || ""}`}
                  </span>
                </div>
              </div>
            );
...
function StatsPage({ goals, habits }) {
  ...
  if (t === 0) return <div style={{ textAlign: "center", padding: "60px 20px" }}><div style={{ fontSize: 48 }}>📊</div><div style={{ fontSize: 15, fontWeight: 700, color: "#94A3B8", marginTop: 8 }}>Создай первую цель</div></div>;

  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      <div style={{ background: "linear-gradient(135deg, #6366F1, #8B5CF6, #A78BFA)", borderRadius: 24, padding: 24, color: "#fff", textAlign: "center" }}>
        <div style={{ display: "flex", justifyContent: "center" }}>
          <Ring pct={pct} size={88} stroke={6} color="#fff"><span style={{ color: "#fff", fontSize: 18, fontWeight: 800 }}>{pct}%</span></Ring>
        </div>
        <div style={{ marginTop: 12, fontSize: 14, fontWeight: 600, opacity: 0.8 }}>Прогресс целей</div>
        <div style={{ display: "flex", justifyContent: "center", gap: 28, marginTop: 16 }}>
          <div><div style={{ fontSize: 24, fontWeight: 800 }}>{d}</div><div style={{ fontSize: 11, opacity: 0.65 }}>Готово</div></div>
          <div><div style={{ fontSize: 24, fontWeight: 800 }}>{t - d}</div><div style={{ fontSize: 11, opacity: 0.65 }}>В работе</div></div>
        </div>
      </div>
      {allT.total > 0 && (
        <div style={{ background: "#fff", borderRadius: 20, padding: "16px 20px", boxShadow: "0 1px 4px rgba(0,0,0,0.04)" }}>
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
            <span style={{ fontSize: 14, fontWeight: 700 }}>Все задачи</span>
            <span style={{ fontSize: 13, fontWeight: 800, color: "#8B5CF6" }}>{allT.done}/{allT.total}</span>
          </div>
      ...
      {byCat.length > 0 && (
        <div style={{ background: "#fff", borderRadius: 20, padding: "16px 20px", boxShadow: "0 1px 4px rgba(0,0,0,0.04)" }}>
          <div style={{ fontSize: 14, fontWeight: 700, marginBottom: 14 }}>По категориям</div>
          {byCat.map((c) => (
            <div key={c.id} style={{ marginBottom: 10 }}>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                <span style={{ fontSize: 13, fontWeight: 650 }}>{c.emoji} {c.label}</span>
                <span style={{ fontSize: 12, fontWeight: 700, color: c.accent }}>{c.d}/{c.t}</span>
              </div>
              <div style={{ height: 6, borderRadius: 3, background: "#F1F5F9", overflow: "hidden" }}>
                <div style={{ height: "100%", borderRadius: 3, width: `${c.t > 0 ? (c.d / c.t) * 100 : 0}%`, background: c.accent, transition: "width .5s" }} />
              </div>
            </div>
          ))}
        </div>
...
export default function App() {
  ...
  const doSave = useCallback(async (g) => {
    try {
      ...
      if (exists) {
        await apiFetch(`/api/goals/${g.id}`, { method: "PUT", body: JSON.stringify(g) });
      } else {
        await apiFetch("/api/goals", { method: "POST", body: JSON.stringify(g) });
      }
      await reload();
    } catch (e) { console.error(e); }
    setSheet(false);
  ...
  const doToggle = useCallback(async (id) => {
    ...
    try {
      await apiFetch(`/api/goals/${id}`, { method: "PUT", body: JSON.stringify({ ...g, done: newDone }) });
      await reload();
    } catch (e) { console.error(e); }
  }, [goals, fireConfetti, reload]);
  const doUpdate = useCallback(async (u) => {
    try {
      await apiFetch(`/api/goals/${u.id}`, { method: "PUT", body: JSON.stringify(u) });
      // Sync tasks: we need a smarter approach — for now update local state and re-sync
  ...
  const odN = goals.filter((g) => !g.done && g.deadline && daysLeft(g.deadline) < 0).length;
  ...
  const tabs = [
    ...
    { id: "notif", emoji: "🔔", label: "Увед." },
  ];

  if (!ready) {
    return <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "100vh" }}><div style={{ fontSize: 32 }}>✨</div></div>;
  }
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
          ...
          {tab === "goals" && (
            <button onClick={() => { setEditG(null); setSheet(true); }} style={{
              ...
              fontSize: 24, cursor: "pointer", boxShadow: "0 4px 15px rgba(99,102,241,0.4)",
              display: "flex", alignItems: "center", justifyContent: "center",
            }}>+</button>
          )}
        </div>
      ...
      <div style={{ padding: "0 16px" }}>
        {tab === "goals" && (
          <>
            ...
            {filtered.length === 0 ? (
              <div style={{ textAlign: "center", padding: "50px 20px" }}>
                <div style={{ fontSize: 48 }}>{goals.length === 0 ? "🎯" : "🔍"}</div>
                <div style={{ fontSize: 16, fontWeight: 800, marginTop: 8 }}>
                  {goals.length === 0 ? "Начни с первой цели!" : "Ничего не найдено"}
                </div>
              </div>
            ) : (
        ...
        {tab === "notif" && <NotifPage goals={goals} />}
      </div>
      ...
      <div style={{
        ...
        {tabs.map((t) => (
          <button key={t.id} onClick={() => setTab(t.id)} style={{
            ...
            padding: "4px 4px", position: "relative", minWidth: 0,
          }}>
            <span style={{
              fontSize: 17, filter: tab === t.id ? "none" : "grayscale(1)",
              opacity: tab === t.id ? 1 : 0.5, transition: "all .2s",
            }}>{t.emoji}</span>
            <span style={{ fontSize: 9, fontWeight: tab === t.id ? 800 : 600, color: tab === t.id ? "#6366F1" : "#94A3B8" }}>{t.label}</span>
            {t.id === "notif" && odN > 0 && (
              <div style={{ position: "absolute", top: 0, right: 0, width: 7, height: 7, borderRadius: 4, background: "#EF4444" }} />
            )}
            {tab === t.id && <div style={{ position: "absolute", top: -1, width: 18, height: 3, borderRadius: 2, background: "#6366F1" }} />}
          </button>
        ))}
      </div>
